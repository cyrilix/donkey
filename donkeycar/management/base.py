...
def load_config(config_path):
    ...
    if not os.path.exists(conf):
        logger.error("No config file at location: %s. Add --config to specify\
    ...
    try:
        ...
        logger.error("Exception while loading config from %s", conf)
        return None
...
class CreateCar(BaseCommand):
    ...
    @staticmethod
    def create_car(path, template='donkey2', overwrite=False):
        """
        ...
        logger.info("Creating car folder: {}".format(path))
        path = make_dir(path)

        logger.info("Creating data & model folders.")
        ...
        if os.path.exists(car_app_path) and not overwrite:
            logger.warning('Car app already exists. Delete it and rerun createcar to replace.')
        else:
            logger.info("Copying car application template: %s", template)
            shutil.copyfile(app_template_path, car_app_path)

        if os.path.exists(car_config_path) and not overwrite:
            logger.warning('Car config already exists. Delete it and rerun createcar to replace.')
        else:
            logger.info("Copying car config defaults. Adjust these before starting your car.")
            shutil.copyfile(config_template_path, car_config_path)

        logger.info("Donkey setup complete.")
